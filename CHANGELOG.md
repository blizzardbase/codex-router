...
- **FORK CHANGE, blizzardbase. The ChatGPT session fallback defaults to OFF.**
  ...
  on default. This fork inverts the default, so
  `CODEX_ROUTER_NATIVE_SESSION_FALLBACK=1` turns it ON, and all three service
  generators write the setting explicitly into the generated service file. The
